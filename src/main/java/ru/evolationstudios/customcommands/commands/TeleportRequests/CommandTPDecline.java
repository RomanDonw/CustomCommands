...
public final class CommandTPDecline implements CommandExecutor
{
    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args)
    {
        ...
        if (args.length >= 1)
        {
            ...
            if (index < 0)
            {
                sender.sendMessage(Config.Message.RequestNotFound);
                return true;
            }

            TeleportAccepts.RemoveRequestByIndex(player, index);
            sender.sendMessage(Config.Message.DeclinedRequestFromPlayer.replaceAll("%PLAYER_NAME%", src_player.getName()));
        }
        else
        {
            ...
            TeleportAccepts.RemoveLastRequest(player);
            sender.sendMessage(Config.Message.DeclinedLastRequest);
        }
